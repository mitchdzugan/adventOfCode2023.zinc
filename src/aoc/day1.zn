...
(defn firstDigit [src origin useDigitSrs]
  (let [start (impl [OriginT origin] (Front 0) (Back (- (.-length src) 1)))
    ...
    (loop [result +/None index start]
      (if (or (not (+/empty? result)) (>= index (.-length src)) (< index 0))
        result
        (recur (getDigit src index useDigitSrs exclude0) (+ index delta))))))

(defn getCalibrationValue [src useDigitSrs]
  (<<- (#(+/bind %2 %1) (firstDigit src Front useDigitSrs)) (fn [dTens])
       (#(+/bind %2 %1) (firstDigit src Back  useDigitSrs)) (fn [dOnes])
       (+/Just (+ (* 10 dTens) dOnes))))

(defn getCalibrationSum [input useDigitSrs]
  (->> (lib/strsplit input "\n")
       (+/fmap #(getCalibrationValue % useDigitSrs))
       (+/reduce #(+ %1 (+/or 0 %2)) 0)))
